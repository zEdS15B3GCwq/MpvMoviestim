...
def main() -> None:
    ...
    direct_render = not RENDER_ON_UPDATE and (
        TARGET_RECT is None
        or TARGET_RECT
        == (
            0,
    ...
    print(
        f"FPS: {player.container_fps=}, {player.estimated_vf_fps=}, {player.display_fps=}"
    )
    ...
    # Main render loop
    # clock = core.Clock()
    global t_render, t_flip, t_draw, need_render, render_count
    # frame_times = np.zeros((10000,), dtype=np.float32)
    frame_count = 0
    big_long_render = []
    big_long_flip = []
    player.pause = False
    ...
    try:
        while not abort_loop:
            ...
            if direct_render:
                ...
                if DO_TIMING:
                    ...
                    if t_render[-1] > 1 / 144:
                        big_long_render.append(
                            [frame_count, round(t_render[-1] * 1000, 3)]
                        )
                render_count += 1
            ...
            if DO_TIMING:
                ...
                if t_flip[-1] > 1 / 144:
                    big_long_flip.append([frame_count, round(t_flip[-1] * 1000, 3)])
        ...
        if DO_TIMING and len(t_render) > 0 and len(t_flip) > 0:
            ...
            print(f"Total render count: {render_count} / {frame_count} frames")
            print(f"Total update_count: {update_count}")

            t_render = np.array(t_render)
            t_flip = np.array(t_flip)
            t_frames = 1000 * np.diff(t_draw)
            frame_histcount = np.histogram(
                t_frames, bins=[0, 0.1, 1, 2, 4, 6, 7, 8, 10, 20, 100, 100000]
            )
