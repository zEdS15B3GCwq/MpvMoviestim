...
GL_ALL_ATTRIB_BITS = 0x000FFFFF  # OpenGL constant for all state bits
...
def _get_integer(name):
    v = (gl.GLint * 1)()
    gl.glGetIntegerv(name, v)
    return int(v[0])


def save_minimal_gl_state():
    return {
        "fb": _get_integer(gl.GL_FRAMEBUFFER_BINDING),
        "prog": _get_integer(gl.GL_CURRENT_PROGRAM),
        "act_tex": _get_integer(gl.GL_ACTIVE_TEXTURE),
        "tex2d": _get_integer(gl.GL_TEXTURE_BINDING_2D),
        "blend": bool(_get_integer(gl.GL_BLEND)),
        "viewport": tuple((gl.GLint * 4)()),
    }


def restore_minimal_gl_state(state):
    gl.glBindFramebuffer(gl.GL_FRAMEBUFFER, state["fb"])
    gl.glUseProgram(state["prog"])
    gl.glActiveTexture(state["act_tex"])
    gl.glBindTexture(gl.GL_TEXTURE_2D, state["tex2d"])
    if state["blend"]:
        gl.glEnable(gl.GL_BLEND)
    else:
        gl.glDisable(gl.GL_BLEND)
    # re-assert viewport if necessary:
...
def main():
    ...
    try:
        while True:
            ...
            state = save_minimal_gl_state()

            # Save OpenGL state before MPV renders
            # More targeted state preservation (if GL_ALL_ATTRIB_BITS is too slow)
            ...
            # )
            # gl.glPushClientAttrib(gl.GL_CLIENT_ALL_ATTRIB_BITS)  # Save client state too

            # Set viewport (optional - for positioning video within window)
            gl.glViewport(viewport_x, viewport_y, viewport_width, viewport_height)

            print(f"pre render state: {save_minimal_gl_state()}")
            ...
            print(f"post render state: {save_minimal_gl_state()}")
            restore_minimal_gl_state(state)

            # # Restore OpenGL state for PsychoPy rendering
            # doesn't work, maybe window is closed already at EOS?
