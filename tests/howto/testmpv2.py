...
def get_proc_address(ctx, name):
    """OpenGL function address resolver for MPV."""
    func_name = name if isinstance(name, bytes) else name.encode("utf-8")
    return gl.current_context.get_proc_address(func_name)
...
def main():
    ...
    print(f'current context: {gl.current_context}')
    print(f'getprocadd: {gl.current_context.get_proc_address}')
    return
